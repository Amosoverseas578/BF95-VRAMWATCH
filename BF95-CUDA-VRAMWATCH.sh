...
# BF95 CUDA VRAMWATCH - compact NVIDIA-SMI + ComfyUI memory dashboard for CUDA GPUs on Linux
# Ubuntu-focused CUDA port. Uses nvidia-smi and Linux /proc telemetry.
#
# Examples:
#   ./BF95-CUDA-VRAMWATCH.sh
#   ./BF95-CUDA-VRAMWATCH.sh --gpu 1 --interval 1
#   ./BF95-CUDA-VRAMWATCH.sh --once
#   NO_COLOR=1 ./BF95-CUDA-VRAMWATCH.sh
...
NVIDIA_HAS_MEMORY_TEMP=0

usage() {
    cat <<'USAGE'
Usage: BF95-CUDA-VRAMWATCH.sh [options]
...
command -v nvidia-smi >/dev/null 2>&1 || die "nvidia-smi was not found in PATH; install/load the NVIDIA driver first"
...
# Memory temperature is exposed only on some NVIDIA GPUs/drivers (commonly
# data-centre/HBM products). Probe it once so unsupported consumer cards still work.
if nvidia-smi --query-gpu=temperature.memory --format=csv,noheader,nounits \
        --id="$GPU_ID" >/dev/null 2>&1; then
    NVIDIA_HAS_MEMORY_TEMP=1
fi
...
load_gpu_metrics() {
    local output query app_output gpu_mem raw_name raw_temp raw_gpu_util raw_mem_util
    local raw_used raw_total raw_free raw_power raw_power_limit raw_gfxclk raw_memclk
    local raw_fan raw_pstate raw_mem_temp

    query='name,temperature.gpu,utilization.gpu,utilization.memory,memory.used,memory.total,memory.free,power.draw,power.limit,clocks.gr,clocks.mem,fan.speed,pstate'
    if ((NVIDIA_HAS_MEMORY_TEMP)); then
        query+=',temperature.memory'
    fi

    if ! output=$(nvidia-smi --query-gpu="$query" --format=csv,noheader,nounits \
            --id="$GPU_ID" 2>&1); then
        METRIC_ERROR=$output
        return 1
    fi
    ...
    IFS=',' read -r raw_name raw_temp raw_gpu_util raw_mem_util raw_used raw_total \
        raw_free raw_power raw_power_limit raw_gfxclk raw_memclk raw_fan raw_pstate \
        raw_mem_temp <<<"$output"

    GPU_NAME=$(trim_field "$raw_name")
    TEMP_GPU=$(value_or_na "$(trim_field "$raw_temp")")
    GPU_UTIL=$(value_or_na "$(trim_field "$raw_gpu_util")")
    MEM_UTIL=$(value_or_na "$(trim_field "$raw_mem_util")")
    VRAM_USED=$(value_or_na "$(trim_field "$raw_used")")
    VRAM_TOTAL=$(value_or_na "$(trim_field "$raw_total")")
    VRAM_FREE=$(value_or_na "$(trim_field "$raw_free")")
    POWER_VAL=$(value_or_na "$(trim_field "$raw_power")")
    POWER_LIMIT=$(value_or_na "$(trim_field "$raw_power_limit")")
    GFXCLK_VAL=$(value_or_na "$(trim_field "$raw_gfxclk")")
    MEMCLK_VAL=$(value_or_na "$(trim_field "$raw_memclk")")
    FAN_PCT=$(value_or_na "$(trim_field "$raw_fan")")
    PSTATE=$(trim_field "$raw_pstate")
    [[ -n "$PSTATE" && "$PSTATE" != "[N/A]" ]] || PSTATE='N/A'

    TEMP_VRAM='N/A'
    if ((NVIDIA_HAS_MEMORY_TEMP)); then
        TEMP_VRAM=$(value_or_na "$(trim_field "$raw_mem_temp")")
    fi

    VRAM_PCT=$(calc_pct "$VRAM_USED" "$VRAM_TOTAL")
    POWER_PCT=$(calc_pct "$POWER_VAL" "$POWER_LIMIT")

    # Process-specific VRAM is useful when GNOME or another CUDA process also
    # occupies the selected GPU. Unsupported/MIG cases simply display N/A.
    COMFY_GPU_MEM_MIB='N/A'
    if [[ -n "$COMFY_PID" ]]; then
        app_output=$(nvidia-smi --query-compute-apps=pid,used_gpu_memory \
            --format=csv,noheader,nounits --id="$GPU_ID" 2>/dev/null || true)
        if [[ -n "$app_output" ]]; then
            gpu_mem=$(awk -F',' -v wanted="$COMFY_PID" '
                {
                    gsub(/^[[:space:]]+|[[:space:]]+$/, "", $1)
                    gsub(/^[[:space:]]+|[[:space:]]+$/, "", $2)
                    if ($1 == wanted && $2 ~ /^[0-9]+([.][0-9]+)?$/) sum += $2
                }
                END { if (sum > 0) printf "%.0f", sum }
            ' <<<"$app_output")
            [[ -n "$gpu_mem" ]] && COMFY_GPU_MEM_MIB=$gpu_mem
        fi
    fi
}
...
draw_dashboard() {
    ...
    printf '%s%sBF95 CUDA VRAMWATCH%s  %sGPU %s%s  %s%s%s  %s
' \
    ...
    if [[ -n "$METRIC_ERROR" ]]; then
        printf '
%s%sNVIDIA-SMI ERROR%s
' "$BOLD" "$RED" "$RESET"
        printf '  Could not read GPU %s. nvidia-smi reported:

' "$GPU_ID"
        printf '  %s
' "$METRIC_ERROR"
        printf '
  Check the GPU index with: nvidia-smi -L
'
        return
    fi
...
' \
    ...
    meter 'Fan' '' "$FAN_PCT" "$CYAN"
    meter 'Power' "${POWER_VAL} W / ${POWER_LIMIT} W" "$POWER_PCT" "$YELLOW"
    printf '  %-12s %-23s         %sMem Clock: %s MHz%s
' \
        'GFX Clock' "${GFXCLK_VAL} MHz" "$DIM" "$MEMCLK_VAL" "$RESET"
    printf '  %-12s %s
' 'Perf State' "$PSTATE"
